...
exports.handler = async function(event, context, callback) {
  ...
  if (conn == null) {
    ...
    conn.model('pulse', new mongoose.Schema({
      variable: String,
      day: String,
      month: String,
      year: String,
  ...
  const M = conn.model('pulse');
  
  let currentDate = new Date();
  currentDate.setHours(currentDate.getHours() - 6);
  
  // a document instance
  var pulse = new M({ variable: data, day: currentDate.getDate(), month: currentDate.getMonth() + 1, year: currentDate.getFullYear(), hour: currentDate.getHours(), minute: currentDate.getMinutes(), second: currentDate.getSeconds()});

  console.log('This is sound1: ', pulse);
  
  conn.collection('pulse').insertOne(pulse);
  
  const doc = await M.findOne(pulse);
  console.log(doc);
